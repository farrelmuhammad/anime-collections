...
interface HeaderProps {
    theme: string; // Change the type to the appropriate one based on your usage
    position: string; // Change the type to the appropriate one based on your usage
}

const Header: React.FC<HeaderProps> = ({ theme, position }) => {
    const [toggleMainMenu, setToggleMainMenu] = useState<boolean>(false);
    return (
        <>
                    ...
                    <div className="flex flex-stretch items-center">
                        <div className="w-56 items-center flex">
                            <Link to="/">
                                <img
                                    // src={Logo}
                                    src="https://tv.idlixprime.com/wp-content/uploads/2020/07/logov4.png"
                                    alt="IDLIX"
                                />
                            </Link>
                        </div>
                        ...
                        <div className="w-auto">
                            <ul
                                className={["fixed shadow-2xl bg-white inset-0 flex flex-col items-center justify-center opacity-0 md:visible md:flex-row md:bg-transparent md:relative md:opacity-100 md:flex md:items-center", toggleMainMenu ? "opacity-100 z-30 visible" : "invisible opacity-0"].join(" ")}
                                id="menu"
                            >
